/**
 * AI-STACK-PATCH (v0.8.7): parseTextParts(..., true) statt Default false —
 * sonst liest TTS den Reasoning-/Think-Block mit vor (Upstream-Bug, Stand
 * 07/2026 auch auf main). Original aus dem Image kopiert, nur diese eine
 * Stelle geändert. Beim Image-Update: Patch gegen neue Version prüfen!
 * Eingebunden als ro-Mount in compose/base.yml.
 */
...
function createChunkProcessor(user, messageId) {
  ...
  async function processChunks() {
    ...
    if (!message) {
      // AI-STACK-PATCH (2 Fixes): (a) skipReasoning=true, damit der
      // Think-Block nicht vorgelesen wird; (b) den hier geparsten Text auch
      // tatsächlich verwenden — das Original las danach wieder das rohe
      // message.text aus der DB (Upstream: parseTextParts-Ergebnis wurde nur
      // gecacht, nie benutzt).
      const dbMessage = await getMessage({ user, messageId });
      if (dbMessage) {
        message = {
          text:
            dbMessage.content?.length > 0
              ? parseTextParts(dbMessage.content, true)
              : dbMessage.text,
          complete: true,
        };
        messageCache.set(cacheKey, message, Time.FIVE_MINUTES);
      }
    }
