/**
 * AI-STACK-PATCH (v0.8.7): Serverseitiger Think-Filter für die manuelle
 * TTS-Route (/tts/manual). Der Klick-Pfad schickt client-seitig gebauten
 * Text, der je nach Client-Version den Reasoning-/Think-Block enthält
 * (ohne <think>-Tags!). Dieser Patch filtert am Server — unabhängig von
 * Browser-/Service-Worker-Caches:
 *   1. Exakt-Abgleich gegen die letzten Nachrichten des Users in der DB:
 *      Input == parseTextParts(content, false) → ersetze durch
 *      parseTextParts(content, true) (nur die Antwort).
 *   2. Fallback: rohe <think>/<thinking>-Tags werden per Regex entfernt.
 * Toggle: TTS_READ_THINK=true (Container-Env) deaktiviert den Filter.
 * Original aus dem Image kopiert; Änderungen: dieser Header, Imports
 * (parseTextParts, getMessages), Methode stripThink, Aufruf in
 * processTextToSpeech. Beim Image-Update Patch gegen neue Version prüfen!
 */
...
class TTSService {
   ...
   */
  /**
   * AI-STACK-PATCH: entfernt den Think-Block aus dem TTS-Input.
   * @param {string} rawInput - Client-Text der manuellen TTS-Route
   * @param {string} userId - User der Anfrage (für den DB-Abgleich)
   * @returns {Promise<string>} gefilterter Input (nie leer: Fallback = Original)
   */
  async stripThink(rawInput, userId) {
    const tagPattern = /<think(?:ing)?>[\s\S]*?<\/think(?:ing)?>/g;
    let result = String(rawInput).replace(tagPattern, '').trim();
    try {
      const normalize = (s) => String(s).replace(/\s+/g, ' ').trim();
      const target = normalize(rawInput);
      const recent = await getMessages({ user: userId, isCreatedByUser: false }, 'content', {
        sort: { createdAt: -1 },
        limit: 25,
      });
      for (const message of recent ?? []) {
        ...
        if (normalize(parseTextParts(message.content, false)) === target) {
          result = parseTextParts(message.content, true).trim();
          logger.debug('[stripThink] Think-Block via DB-Abgleich entfernt');
          break;
        }
      }
    } catch (error) {
      logger.warn('[stripThink] DB-Abgleich übersprungen:', error);
    }
    return result.length > 0 ? result : String(rawInput);
  }
  ...
  async processTextToSpeech(req, res) {
    ...
    /** AI-STACK-PATCH: Think-Filter (Toggle: TTS_READ_THINK=true deaktiviert) */
    const input =
      process.env.TTS_READ_THINK === 'true'
        ? rawInput
        : await this.stripThink(rawInput, req.user?.id);
